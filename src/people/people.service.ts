...
export class PeopleService {
  ...
  async getPersonById(id: number): Promise<PersonResponse> {
    try {
      ...
      if (!person) {
        console.log(`Person with ID ${id} not found in database. Fetching from external API...`);
        ...
        console.log(`Person with ID ${id} saved to database`);
        return formattedData;
      } else {
        console.log(`Person with ID ${id} found in database`);
      }
